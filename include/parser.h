...
private:
     ...
     */
    std::vector<std::vector<Token>> splitByPipe(const std::vector<Token>& tokens);
     ...
     */
    bool validatePipeline(const std::vector<std::vector<Token>>& commandTokens, std::string& errorMessage);
     ...
     */
    std::unique_ptr<AbstractCommand> parseSingleCommand(const std::vector<Token>& tokens);
