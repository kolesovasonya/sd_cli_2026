...
2.  **Lexer:**
    ...
    *   **Особенности:**
        ...
        *   **Пайпы:** Распознает оператор `|` как отдельный токен (тип `PIPE`).
...
3.  **Parser:**
    ...
    *   **Детали парсинга:**
        *   **Подстановки:** На этом этапе происходит разрешение переменных окружения. Токены, помеченные лексером как потенциальные переменные окружения, направляются в `Environment Manager` для получения их значений. Если переменная не найдена, она может быть заменена пустой строкой или вызывать ошибку. Поддерживается специальная переменная `$?` для получения exit code последней команды.
    ...
    *   **Объектное представление результата разбора:** Создает иерархию наследников `AbstractCommand` для выполнения.
        *   `BuiltinCommand`: Базовый класс для встроенных команд (cat, wc, echo, pwd, exit, grep).
...
4.  **Environment Manager:**
    ...
    *   **Специальная переменная `?`:** После каждой команды в `main()` сохраняется exit code в переменную `?`, доступную через `$?`.
...
*   **Наследники `BuiltinCommand`:**
    *   `CatCommand`: Открывает файл, читает содержимое, пишет в `stdout`. Если файл не указан или `-`, читает из `stdin`.
    *   `WcCommand`: Открывает файл, считает строки/слова/байты, пишет в `stdout`. Если файл не указан, читает из `stdin`.
    ...
    *   `GrepCommand`: Ищет строки, совпадающие с регулярным выражением, в файле или `stdin`. Поддерживает флаги `-w` (поиск только целого слова), `-i` (регистронезависимый поиск), `-A N` (вывод N строк контекста после каждого совпадения; пересекающиеся диапазоны объединяются, несмежные разделяются `--`). Аргументы разбираются библиотекой **cxxopts**, сопоставление выполняется через `std::regex`. Возвращает 0 при наличии хотя бы одного совпадения, 1 — если совпадений нет.
...
*   Коды возврата: Каждая команда должна возвращать целочисленный код возврата (0 для успеха, ненулевое значение для ошибки). `Command Executor` должен сохранять этот код. Exit code последней команды автоматически сохраняется в переменную `?` и доступен через `$?`.
